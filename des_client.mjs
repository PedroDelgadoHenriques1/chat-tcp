...
client.on('data', (data) => {
    ...
    let hexList = stringToHexList(data.toString());
    console.log('hexList', hexList);
    let msgFinal = desDecrypt(hexList, chave);
...
rl.on('line', (input) => {
    const mensagem = '4174616361722062';
    const chave = '0123456789ABCDEF';
    // let x = des(mensagem, chave);
    // console.log('debug', x);
    let debug = stringTo64BitHex('Atacar b')
    console.log('debug', debug)
    let item = criptografiaCompleto('Atacar b', chave);
...
function criptografiaCompleto(input, chave) {
    ...
    const result = list.map(element => {
        ...
        console.log("criptografando", element);

        const final = des(binaryToHex(element), chave);

        console.log("Mensagem criptografada final (hexadecimal):", final);
        return final;
    });
...
function desDecrypt(hexList, chave) {
    ...
    for (let i = 0; i < hexList.length; i += 1) {
        ...
        const chavebinaria = textToBin(chave); // Convert key to binary
        ...
        // Step 4: 16 rounds of DES
        for (let i = 0; i < 16; i++) {
            const tempL = L; // Store L(n) temporarily
            L = R; // L(n+1) = R(n)
            R = xorBinary(tempL, f(R, reversedSubKeys[i])); // R(n+1) = L(n) XOR f(R(n), K(n))
        }
